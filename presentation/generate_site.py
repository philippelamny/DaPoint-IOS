...
def build_html(games, version, releases):
    ...
    if releases:
        ...
        release_items.append('<li>Aucun build disponible pour le moment. Déposez les fichiers dans <code>presentation/releases</code> puis relancez <code>generate_site.py</code>.</li>')
    ...
    if screenshot_files:
        ...
        screenshot_items.append('<div class="screen-card"><p>Aucune capture d\'écran disponible. Lancez l\'application sur l\'émulateur et capturez un screenshot dans <code>presentation/screenshots</code>.</p></div>')
