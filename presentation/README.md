...
2. Ajoutez les fichiers de build dans `presentation/releases/`.
3. Exécutez :
...
Si vous utilisez la tâche VS Code, la commande `Flutter: build APK (release, versioned)` copiera automatiquement l’APK généré dans `presentation/releases/` puis mettra à jour `presentation/index.html`.

4. Publiez la page avec GitHub Pages en utilisant `presentation/` comme dossier source.

## Points importants

- La page affiche automatiquement les jeux détectés dans le registre Swift.
- Les liens de téléchargement sont générés depuis le dossier `presentation/releases`.
